...
//void WavAssembler::GetWaveError(MMRESULT err) {
//    char errorMsg[256];
//    if (err == 0) {
//        return;
//    }else{
...
//void WavAssembler::setPath(QString path) {
//    a=QString(path);
//    this->path=(LPTSTR)a.utf16();
////    this->path=path;
//}

//void WavAssembler::setVolume(int volume) {
...
//    *Open the a wav file.
//    ---------------------------------------------------------------------------------*/
//    if ((WavAssembler::hmmioIn = mmioOpen(path, NULL, MMIO_READ)) == NULL) {
...
//void WavAssembler::resume() {
//    mut.unlock();
//    waveOutRestart(hAudioOut);
//}

//void WavAssembler::subThread(int bufferLoop) {
////    if (!mmioRead(hmmioIn, databuffer[bufferLoop].lpData, bufferSize)) {
////        qDebug() << "Error in reading the waveformat\n";
////        mmioClose(hmmioIn, 0);
////        exit(-1); //exit(-1);
...
////void SubThread::run() {
////#include <WavAssembler.h>
////    if (!mmioRead(*WavAssembler.hmmioIn, databuffer[bufferLoop].lpData, bufferSize)) {
////        qDebug() << "Error in reading the waveformat\n";
////        mmioClose(*WavAssembler.hmmioIn, 0);
////        return; //exit(-1);
////    }
////    if (databuffer[bufferLoop].lpData == 0) {
////        break;
////    }
////    waveOutWrite(*WavAssembler.hAudioOut, &databuffer[bufferLoop], sizeof(WAVEHDR));
////    Sleep(10000);
////    waveOutUnprepareHeader(*WavAssembler.hAudioOut, &databuffer[bufferLoop], sizeof(databuffer[bufferLoop]));
////    memset(databuffer[bufferLoop].lpData, 0, bufferSize);
////    err = waveOutPrepareHeader(*WavAssembler.hAudioOut, &databuffer[bufferLoop], sizeof(WAVEHDR));
////}
